...
#save parameters
dbWriteTable(con,'parameters_lr',parameters,row.name=FALSE,overwrite=TRUE)
#Clear results table
dbSendQuery(con,'truncate table results_lr')
...
apply(
  ...
  function(param){
    ...
    res[[1]]<-cbind(param['id'],seq.int(nrow(res[[1]])),res[[1]])
    names(res[[1]])<-c('parameters_id','individual_id','x','y','rank','crowding','generation')
    dbWriteTable(con,'results_lr',res[[1]],row.name=FALSE,append=TRUE)
    #individuals (points)
